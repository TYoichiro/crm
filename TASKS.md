...
- [x] ログイン画面（ID入力→Cookie保存→ダッシュボードへ遷移）
- [x] ダッシュボード（KPIカード＋ステータス別内訳＋直近活動）
- [x] 顧客一覧（検索付き、新規作成モーダル付き）
- [x] 顧客詳細（基本情報＋商談リスト＋活動履歴タイムライン）
- [x] 商談パイプライン（カンバン、横スクロール、ステータス変更、新規作成モーダル付き）
- [x] 商談詳細（編集フォーム＋削除）
- [x] 活動の追加/編集モーダル
